...
import { loadScope, allowedBrands } from '../auth/middleware.js'
 ...
 * Who gets what.
 *
 * Scope is read from the database exactly as the API reads it, so an email can
 * never show someone a branch the dashboard would refuse them. A store user
 * with no location grant would otherwise receive every branch in the brand,
 * which is why that case is skipped rather than widened.
 */
export function recipients() {
  const users = db
    .prepare(
      `SELECT id, email, name, role FROM users
        WHERE status = 'active' AND role IN ('store', 'stakeholder')
        ORDER BY role, email`
    )
    .all()
  ...
  return users
    .map((u) => {
      const scope = loadScope(u.id, u.role)
      const brands = allowedBrands(scope)
      const locations = scope.locations ? [...scope.locations] : null

      if (!brands.length) return { ...u, skip: 'no brand granted' }
      if (u.role === 'store' && !locations?.length) {
        return { ...u, skip: 'no store granted — a store report needs one branch' }
      }
      return { ...u, brands, locations }
    })
    .filter(Boolean)
}

/**
 * Builds the messages for one explicit recipient.
 *
 * Scope comes from the recipient row an administrator set, not from a user
 * account — a distribution list has no account, and that is a legitimate
 * address to send a branch's prep list to.
 */
/* ---------------------------------------------------- branch forecast ---- */

/**
 * Tomorrow's forecast for one branch, at both levels it is actually used at.
 *
 * A branch is identified the way the business says it out loud — BBT-BYN — and
 * gets two tables rather than one:
 *
...
function groupByProduct(rows) {
  ...
  for (const r of rows) {
    ...
    const prev = map.get(key) ?? { product: key, forecast: 0, recent: 0, articles: 0, extra: 0 }
  ...
  return [...map.values()]
    .map((p) => ({ ...p, change: p.recent ? (p.forecast - p.recent) / p.recent : null }))
    .sort((a, b) => b.forecast - a.forecast)
}

export function branchForecastHtml({ brand, location, rows, day, trust, prepared = [], attached = [] }) {
  const products = groupByProduct(rows)
  const units = sumQty(rows)
  const extra = countExtra(rows)
  const biggest = products[0]
  const preparedUnits = prepared.reduce((n, r) => n + (Number(r.Component_Forecast_Qty) || 0), 0)
  ...
  const body = `
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="margin:-6px -6px 18px;">
      <tr>
        ...
        ${statCard(
          ...
          prepared.length ? 'Items made in house' : 'Demand up sharply'
        )}
      </tr>
    </table>

    ${lead(
      `Tomorrow’s forecast for <strong>${esc(brand.code)}-${esc(location)}</strong>: ` +
        `${int(units)} units across ${int(products.length)} product${products.length === 1 ? '' : 's'}` +
        (biggest
          ? `, led by <strong>${esc(biggest.product)}</strong> at ${int(biggest.forecast)}`
          : '') +
        (attached.length
          ? `. The full figures are in the attached workbook${attached.length === 1 ? '' : 's'}.`
          : '.')
    )}
    ...
    ${
      prepared.length
        ? `<div style="font-size:13px;color:${INK};margin-top:16px;line-height:1.55;">
             <strong>${int(prepared.length)} item${prepared.length === 1 ? '' : 's'}</strong> are made in the
             kitchen rather than taken off a shelf &mdash; about ${int(preparedUnits)} units in total. The
             quantities tomorrow’s forecast implies are on the <em>To prepare</em> tab.
           </div>`
        : ''
    }
  ...
  return shell(`${brand.code}-${location} · forecast for tomorrow`, longDate(day), body)
}
...
/** Builds the message for one recipient, or null if there is nothing to send. */
export async function buildFor(user) {
  if (user.skip) return null

  if (user.role === 'store') {
    // One brand, one branch. If a store account somehow spans several, each
    // branch gets its own message rather than one confusing merged list.
    const messages = []
    for (const brand of user.brands) {
      const plan = await planFor(brand, user.locations)
      for (const location of user.locations) {
        const rows = plan.rows.filter((r) => String(r.LocationID) === String(location))
        if (!rows.length) continue
        const day = tomorrowFrom(plan.today)
        messages.push({
          subject: `${location} · prep for tomorrow, ${longDate(day)}`,
          html: storeReportHtml({ brand, location, rows, day, trust: plan.trust }),
          meta: { brand: brand.code, location, rows: rows.length, units: sumQty(rows) },
        })
      }
    }
    return messages
  ...
  return [
    {
      subject: `Tomorrow across your stores · ${longDate(day)}`,
      // The largest brand's trust figure, since a single sentence cannot honestly
      // average two models with different accuracies.
      html: stakeholderReportHtml({
        brands,
        day,
        trust: [...brands].sort((a, b) => sumQty(b.rows) - sumQty(a.rows))[0]?.trust ?? null,
      ...
      meta: {
        brands: brands.map((b) => b.brand.code),
        stores: new Set(brands.flatMap((b) => b.rows.map((r) => r.LocationID))).size,
      },
    },
  ]
}
